...
static glm::vec3 ansi_to_rgb(int n)
{
  switch(n)
    ...
    case 31: return {.8f, 0, 0};
    case 32: return {0, .8f, 0};
    case 33: return {.8f, .8f, 0};
    case 34: return {0, 0, .8f};
    case 35: return {.8f, 0, .8f};
    case 36: return {0, .8f, .8f};
    case 37: return {.8f, .8f, .8f};
    case 90: return {.5f, .5f, .5f};
    case 91: return {1, 0, 0};
    case 92: return {0, 1, 0};
    case 93: return {1, 1, 0};
    case 94: return {.3f, .3f, 1};
    case 95: return {1, 0, 1};
    case 96: return {0, 1, 1};
    case 97: return {1, 1, 1};
  ...
  for(unsigned char c = 32; c < 127; ++c)
    {
      ...
      for(int j = 0; j < h; ++j)
        for(int i = 0; i < w; ++i)
          px[(y + j) * ATLAS_WIDTH + (x + i)] = face->glyph->bitmap.buffer[j * w + i];
      Glyph g;
...
static void grid_resize_locked()
{
  ...
  auto old = g_term.grid;
  int oc = g_term.cols, or_ = g_term.rows;
  g_term.cols = nc;
  g_term.rows = nr;
  ...
  while(std::getline(ss, tok, ';'))
    {
      ...
      switch(n)
        ...
        case 0:
          ...
          g_ansi.italic = false;
          break;
        case 1: g_ansi.bold = true; break;
        case 3: g_ansi.italic = true; break;
        case 22: g_ansi.bold = false; break;
        case 23: g_ansi.italic = false; break;
        case 39: g_ansi.fg = {0, .8f, .6f}; break;
        case 49: g_ansi.bg = {0, 0, 0}; break;
        default:
          if((n >= 30 && n <= 37) || (n >= 90 && n <= 97))
            g_ansi.fg = ansi_to_rgb(n);
          else if(n >= 40 && n <= 47)
            g_ansi.bg = ansi_to_rgb(n - 10);
          else if(n >= 100 && n <= 107)
            g_ansi.bg = ansi_to_rgb(n - 70);
          break;
        }
    }
}
...
static void process_csi_locked(const std::string &params, char fb)
{
  auto parse = [&](int def) -> std::vector<int> {
    ...
    while(std::getline(ss, t, ';'))
      v.push_back(t.empty() || t == "0" ? def : std::stoi(t));
  ...
  row = std::clamp(row, 0, R - 1);
  col = std::clamp(col, 0, C - 1);
  switch(fb)
    {
    ...
    case 'H':
        ...
        if(n == 0)
          {
            ...
            for(int r = row + 1; r < R; ++r)
              grid[r].assign(C, Cell{});
          }
        else if(n == 1)
          {
            for(int r = 0; r < row; ++r)
              grid[r].assign(C, Cell{});
            for(int c = 0; c <= col && c < C; ++c)
              grid[row][c] = Cell{};
          }
        else if(n == 2 || n == 3)
          {
      ...
      case 'K': {
        ...
        if(n == 0)
          ...
          grid[row].assign(C, Cell{});
        break;
      }
    case 'L':
      for(int i = 0; i < P(0, 1); ++i)
        {
          grid.insert(grid.begin() + row, std::vector<Cell>(C));
    ...
    case 'T':
      for(int i = 0; i < P(0, 1); ++i)
        {
          grid.pop_back();
          grid.insert(grid.begin(), std::vector<Cell>(C));
        }
      break;
    case 'X':
      for(int i = 0; i < P(0, 1) && col + i < C; ++i)
        grid[row][col + i] = Cell{};
      break;
    case 'h':
    case 'l':
  ...
  for(size_t i = 0; i < size; ++i)
    {
      ...
      switch(g_ansi.state)
        ...
        case EscState::Normal:
          if(byte == 0x1B)
            {
              g_ansi.state = EscState::Esc;
              g_ansi.param_buf.clear();
            }
          else
            grid_put_locked((char)byte);
          break;
        case EscState::Esc:
          g_ansi.param_buf.clear();
          if(byte == '[')
            g_ansi.state = EscState::CSI;
          else if(byte == ']')
            g_ansi.state = EscState::OSC;
          else if(byte == 'c')
            {
              for(auto &r : g_term.grid)
                r.assign(g_term.cols, Cell{});
              g_term.cur_row = 0;
              g_term.cur_col = 0;
            ...
            {
              if(g_term.cur_row > 0) --g_term.cur_row;
              g_ansi.state = EscState::Normal;
            }
          else
            g_ansi.state = EscState::Normal;
        ...
        case EscState::CSI:
          if(byte >= 0x40 && byte <= 0x7E)
            ...
            g_ansi.param_buf += (char)byte;
          break;
        case EscState::OSC:
          if(byte == 0x07)
            {
              process_osc(g_ansi.param_buf);
              g_ansi.state = EscState::Normal;
              g_ansi.param_buf.clear();
            }
          else if(byte == 0x1B)
...
void build_terminal_vertices(std::vector<float> &verts, int W, int H)
{
  ...
  Cell cursorCell;
  {
    ...
    if(offset == 0)
    ...
    else
      {
        ...
        snap.reserve(rows);
        for(int i = sbStart; i < sbStart + sbRows; ++i)
          snap.push_back(g_term.scrollback[i]);
        for(int i = 0; i < liveRows; ++i)
          snap.push_back(g_term.grid[i]);
        cursorRow = sbRows + g_term.cur_row;
      }
    cursorCol = g_term.cur_col;
    int cr = g_term.cur_row;
    if(cr >= 0 && cr < (int)g_term.grid.size() && cursorCol >= 0 && cursorCol < (int)g_term.grid[cr].size())
    ...
    for(int col = 0; col < (int)snap[row].size(); ++col)
      {
        ...
        if((bg.r + bg.g + bg.b) > 0.01f)
          {
            emit(nx0, ny0, 0, 0, bg, z, 0, 96.f);
            emit(nx1, ny0, 0, 0, bg, z, 0, 96.f);
            emit(nx1, ny1, 0, 0, bg, z, 0, 96.f);
            emit(nx0, ny0, 0, 0, bg, z, 0, 96.f);
            emit(nx1, ny1, 0, 0, bg, z, 0, 96.f);
            emit(nx0, ny1, 0, 0, bg, z, 0, 96.f);
          }
        ...
        // Pick glyph map
        int gi = 0;
        if(cell.bold && cell.italic && g_bold_italic_face)
          gi = 3;
        else if(cell.bold && g_bold_face)
          gi = 1;
        else if(cell.italic && g_italic_face)
          gi = 2;

        auto it = g_glyphs[gi].find(ch);
        if(it == g_glyphs[gi].end())
          {
            it = g_glyphs[0].find(ch);
            gi = 0;
          }
  ...
  if(cursor_visible() && g_scroll_offset == 0)
    {
      float x0 = cursorCol * cellW, y0 = cursorRow * cellH, x1 = x0 + cellW, y1 = y0 + cellH;
      float nx0 = ndcX(x0), nx1 = ndcX(x1), ny0 = ndcY(y0), ny1 = ndcY(y1);
      ...
      if(ch != ' ' && ch != '\0')
        {
          auto it = g_glyphs[0].find(ch);
          if(it != g_glyphs[0].end())
            {
              const Glyph &g = it->second;
              float gx0 = x0 + g.bearingX, gy0 = y0 + ascent - g.bearingY;
              float gx1 = gx0 + g.width, gy1 = gy0 + g.height;
...
void reload_font_size(int sz)
{
  ...
  pty.resize(g_term.cols, g_term.rows);
  g_dirty = true;
}
  ...
  if(mods & GLFW_MOD_CONTROL)
    {
      switch(key)
        ...
        case GLFW_KEY_EQUAL:
        case GLFW_KEY_KP_ADD: reload_font_size(FONT_SIZE + 2); return;
        case GLFW_KEY_MINUS:
        case GLFW_KEY_KP_SUBTRACT: reload_font_size(FONT_SIZE - 2); return;
        case GLFW_KEY_0:
        case GLFW_KEY_KP_0: reload_font_size(18); return;
        default: break;
        }
    }
...
static void scroll_callback(GLFWwindow *, double, double yoff)
{
  ...
  {
    std::lock_guard<std::mutex> lk(g_lock);
    max_offset = (int)g_term.scrollback.size();
  }
  if(max_offset == 0)
    {
      accum = 0;
  ...
  {
    std::lock_guard<std::mutex> lk(g_lock);
    g_scroll_offset = std::clamp(g_scroll_offset - step, 0, max_offset);
  }
  g_dirty = true;
}
...
int main()
{
  ...
  if(FT_New_Face(g_ft, "JetBrainsMono-Medium.ttf", 0, &g_face))
    {
  ...
  setSize(g_face);
  setSize(g_bold_face);
  setSize(g_italic_face);
  setSize(g_bold_italic_face);
  load_glyphs();
  ...
  while(!glfwWindowShouldClose(win))
    {
      ...
      glBindTexture(GL_TEXTURE_2D, g_atlasTex[0]);
      glActiveTexture(GL_TEXTURE1);
      glBindTexture(GL_TEXTURE_2D, g_atlasTex[1]);
      glActiveTexture(GL_TEXTURE2);
      glBindTexture(GL_TEXTURE_2D, g_atlasTex[2]);
      glActiveTexture(GL_TEXTURE3);
      glBindTexture(GL_TEXTURE_2D, g_atlasTex[3]);
      glBindVertexArray(g_termVAO);
