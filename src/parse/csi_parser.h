...
namespace LSD::CsiParser
{
    static void process_csi_locked(const std::string &params, char fb) {
        ...
        int &col = g_term.cur_col;
        int &row = g_term.cur_row;
        const int C = g_term.cols;
        const int R = g_term.rows;
        ...
        switch (fb) {
            ...
            case 'K': {
                ...
                if(n == 0) {
                    ...
                    grid[row].assign(C, LSD::Types::Cell{});
                }
                break;
            }
            case 'L': 
            ...
            case 'S': 
                for(int i = 0; i < P(0,1); ++i) 
                    grid_scroll_up_locked();  // Now should be found
                break;
            case 'T': 
                for(int i = 0; i < P(0,1); ++i) {
                    grid.pop_back();
