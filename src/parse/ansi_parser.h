...
namespace LSD::AnsiParser
{
    ...
    static void apply_sgr(const std::string &params)
    {
        if (params.empty() || params == "0") {
            g_ansi_state.fg = {1,1,1}; g_ansi_state.bg = {0,0,0};
            g_ansi_state.bold = false; g_ansi_state.italic = false; return;
        }
        ...
        while (std::getline(ss, tok, ';'))
        {
            ...
            switch (n) 
            {
                case 0: g_ansi_state.fg={0,.8f,.6f}; g_ansi_state.bg={0,0,0}; g_ansi_state.bold=false; g_ansi_state.italic=false; break;
                case 1: g_ansi_state.bold=true;   break; case 3: g_ansi_state.italic=true;  break;
                case 22:g_ansi_state.bold=false;  break; case 23:g_ansi_state.italic=false; break;
                case 39:g_ansi_state.fg={0,.8f,.6f}; break; case 49:g_ansi_state.bg={0,0,0}; break;
                default:
                    if ((n>=30&&n<=37)||(n>=90&&n<=97)) g_ansi_state.fg = ansi_to_rgb(n);
                    else if (n>=40&&n<=47)  g_ansi_state.bg = ansi_to_rgb(n-10);
                    else if (n>=100&&n<=107)g_ansi_state.bg = ansi_to_rgb(n-70);
                    break;
            }   
        }
    }
